...
typeAssertion :: e ~ Fix (ExprF selId id bin un lit ty) => e -> ty -> e
typeAssertion e ty = Fix $ TypeAssertion e ty

-- | Constructs an expression tree for 'Call'.
call :: e ~ Fix (ExprF selId id bin un lit ty) => e -> Maybe ty -> [e] -> e
call e ty params = Fix $ Call e ty params

-- | Constructs an expression tree for 'Literal'.
literal :: e ~ Fix (ExprF selId id bin un lit ty) => lit -> e
literal = Fix . Literal

-- | Constructs an expression tree for 'Variable'.
variable :: e ~ Fix (ExprF selId id bin un lit ty) => id -> e
